...
struct whisper_params {
    ...
    std::string owts_dimensions = "1200x120";
...
bool whisper_params_parse(int argc, char ** argv, whisper_params & params) {
    for (int i = 1; i < argc; i++) {
        ...
        else if (arg == "-owtsd"|| arg == "--output-words-dim"){ params.owts_dimensions = argv[++i]; }
...
void whisper_print_usage(int /*argc*/, char ** argv, const whisper_params & params) {
    ...
    fprintf(stderr, "  -owtsd,    --output-words-dim  [%-7s] output dimensions for karaoke video widthxheight\n", params.owts_dimensions.c_str());
...
bool output_wts(struct whisper_context * ctx, const char * fname, const char * fname_inp, const whisper_params & params, float t_sec, std::vector<std::vector<float>> pcmf32s) {
    ...
    static const char * dimensions = params.owts_dimensions.c_str();
    ...
    fout << "ffmpeg -i " << fname_inp << " -f lavfi -i color=size=" << dimensions << ":duration=" << t_sec << ":rate=25:color=black -vf \"";
